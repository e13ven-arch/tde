...
export HF_HUB_DISABLE_PROGRESS_BARS=1 PYTHONUNBUFFERED=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
...
cd /workspace/jevbench && rm -rf "$OUT/jevbench" && mkdir -p "$OUT/jevbench"
/workspace/jev/.venv/bin/python -m jevbench.cli run --tasks datasets/public/easy.jsonl,datasets/public/original.jsonl,datasets/public/hard.jsonl \
...
/workspace/jev/.venv/bin/python - "$OUT/jevbench/results.jsonl" <<'PY'
