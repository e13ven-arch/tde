...
第一期与 Qwen 系实现的对比只做推理，不训练：typed-decisions 与 JevBench 的已发布行（kev、SemIf、Laya、verdict-2.0、Jev）；Qwen3-0.6B 零样本 prefill 基线在本项目测试集上的结果；可选地用 verdict-2.0 / Laya / NanoJev 的公开权重在全标签集与相干性测试上推理。Exp 002（同数据同参数量的编码器对解码器全参对照）移至第二期，需 ≥24GB 显存。
