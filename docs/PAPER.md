...
**Reading Decisions out of a Masked Language Model: Readout Position, Objective, and Candidate Count in a 150M Typed-Decision Encoder**
...
- 发现 3：listwise 读出对候选数外推差（−30 点）；训练时见大 K 收回 15 点，推理端分块锦标赛零训练收回一半，二者不叠加。
...
- 3.3 目标函数：soft-CE + RPS + perm-KL，声明均为已有成分并引用来源。
...
## 5 实验
- 5.1 读出位置（表 2：2×2 标记 × span，3 seeds，ModernBERT 与 DeBERTa）→ 待补：Exp 017 / 016。
- 5.2 读出结构（表 3：joint / branch / bi-encoder，含训练成本）→ 已有：Exp 001。
- 5.3 目标函数（表 4：五个 arm 的准确率、NLL、ECE、AURC、一致性、温度）→ 已有：Exp 003。
- 5.4 候选数（表 5：一次前向 / 分块 / 锦标赛 × 训练 K 上限）→ 已有：Stage 1、Exp 006、010。
...
## 6 分析
- 读出位置的机制：[MASK] 位置表示与候选 span 的相似度分析；新增标记的学习曲线（Mac 上 3,200 条 = v1 的 16,000 条）。
...
## 待补实验（进行中）
- Exp 017：标记 × span × 3 seeds（12 次，Stage 0 设置）。
- Exp 016：DeBERTa-v3-base 上 [MASK]+span 与新标记两种读出。
