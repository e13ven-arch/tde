...
5. **Under the production recipe.** Retraining the release configuration (v0.5 data, GLiClass initialisation, 1,024-token states, single seed) under set and pointwise reproduces the full-label gains on top of full-set rendering (banking77 75.8 → 78.3 / 77.9, clinc150 86.5 → 89.6 / 88.9, coverage at 5% risk +4 to +5 points), leaves the in-distribution and v0.1 regression numbers unchanged (83.5 / 87.7 vs 83.4 / 87.5), and drops the permutation-KL forward pass. On JevBench's public hard tier the three topologies are inside the 111-item sampling band (34.2 seq, 30.6 set, 32.4 pointwise, with near-symmetric paired flips); pointwise gains 5 items on the standard tier. Topology therefore buys extrapolation, equivariance and training cost, not benchmark intelligence, which is consistent with §5.5. Option sets *designed* to be compared (typed-decisions in mixture mode) are within 0.2 points for set and 1.6 points lower for pointwise, so set is the safer default when candidate interaction may matter.
