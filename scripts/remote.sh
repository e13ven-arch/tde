...
case "$cmd" in
  sync)
    rsync -az --delete --exclude .venv --exclude data --exclude runs --exclude .git --exclude '__pycache__' ./ "$REMOTE:$REMOTE_DIR/" ;;
  setup)
  ...
  push-data)
    rsync -az data/v0.1/ "$REMOTE:$REMOTE_DIR/data/v0.1/" ;;
  train)
    cfg="$1"; shift
  ...
  pull)
    rsync -az "$REMOTE:$REMOTE_DIR/$1/" "$1/" ;;
  *)
    sed -n 2,9p "$0"; exit 1 ;;
esac
